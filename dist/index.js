...
function fillBoard(aBoardID, aPuzzle5) {
    const tbody = document.getElementById(aBoardID);
    aPuzzle5.board.forEach((aLine)=>{
...
// function moveRobotRandomly(aBoardID) {
//     removeRobot();
//     addRobot(aBoardID,
//              {x: getRandomInt(12),
// 	      y: getRandomInt(16),
...
// }
function Move(aBoardID) {
    removeRobot();
    _robotJs.move(aPuzzle);
    //    Out_of_bound(aPuzzle.robot,aPuzzle);
    addRobot(aBoardID, aPuzzle.robot);
}
function Left(aBoardID) {
    removeRobot();
    _robotJs.left(aPuzzle);
    addRobot(aBoardID, aPuzzle.robot);
}
function Right(aBoardID) {
    removeRobot();
    _robotJs.right(aPuzzle);
    addRobot(aBoardID, aPuzzle.robot);
}
...
},{"../src/robot.js":"iwS1q","../src/interpreter.js":"gTVuZ"}],"iwS1q":[function(require,module,exports) {
...
function apply_inst(puzzle, inst, error_function) {
    if (inst.func !== move && inst.func !== left && inst.func !== right) {
...
function step(puzzle, context, error_function) {
    ...
    context.Pc[1] === 0 && match(func, cell_content);
    //console.log('matched');
    if (typeof inst.func === 'function') {
        //console.log('inst.func = ', inst.func);
        if (match(inst, cell_content)) apply_inst(puzzle, inst);
        context.Pc = [
            fct_indx,
            inst_indx + 1
        ];
        context.nb_steps++;
    } else if (inst.func === -1) //console.log("end of function");
    context.Pc = [
        fct_indx + 1,
        0
    ];
        ...
        } else //console.log("before", context);
        //console.log("unstack = ", S.unstack(context.s));
        context.Pc = _stackJs.unstack(context.s);
    } else //console.log("entered", context.Pc);
    if (match(inst, cell_content)) {
        //console.log("matched", context.Pc);
        context.Pc = [
            fct_indx,
            inst_indx + 1
        ];
        ...
        context.Pc[1] = 0;
        //console.log("find_function", context.Pc);
        context.nb_steps++;
...
function print_puzzle(puzzle) {
    if (is_valid_position(puzzle, puzzle.robot.x, puzzle.robot.y)) {
        ...
        console.log("Invalid robot position or program finished");
        return;
    }
}
...
},{"./robot.js":"iwS1q","./stack.js":"dfUzx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gTVuZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function find_function(program1, fct1) {
    function find_function_rec(program, fct, idx) {
        if (program[idx] === fct) return idx;
            ...
            idx++;
            return find_function_rec(program, fct, idx);
        }
    }
...
function is_over(puzzle, context, error_function) {
    ...
    _robotJs.execution_limits(puzzle, context, error_function);
    return 0;
}
...
function execute_and_print(puzzle, context, error_function) {
    ...
    while(!is_over(puzzle, context)){
        _robotJs.step(puzzle, context, error_function);
        console.log(_puzzleJs.print_puzzle(puzzle));
    }
}

},{"./puzzle.js":"4wns9","./robot.js":"iwS1q","./stack.js":"dfUzx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["3T0ci","7SwCM"], "7SwCM", "parcelRequireb8af")
